...
const userSchema = new mongoose.Schema({
  ...
  mobile: {
    ...
    trim: true,
    match: [/^\+\d{1,4}\s[\d\s\-]{5,20}$/, "Please enter a valid mobile number in format +country code space number"],
    validate: {
      validator: function(v) {
        return /^\+\d{1,4}\s[\d\s\-]{5,20}$/.test(v);
      },
      message: "Please enter a valid mobile number in format +country code space number"
    }
  },
  alternateEmail: {
    type: String,
  ...
  roles: {
    type: [String],
    enum: {
      values: ["buyer", "seller", "partner", "all"],
  ...
  companyName: {
    type: String,
    required: function() {
      return this.roles && this.roles.includes('seller');
    },
    trim: true,
  ...
  // Separate billing addresses (stored as their own subdocument schema)
...
// Removed TTL index on otp.expiresAt to prevent automatic deletion of user documents.
// If OTP expiry TTL behavior is desired, use a separate OTP collection with a TTL index.
...
userSchema.virtual('defaultAddress').get(function() {
  const addrs = Array.isArray(this.addresses) ? this.addresses : [];
  const found = addrs.find(function(addr) { return addr && addr.isDefault; });
  return found || (addrs.length > 0 ? addrs[0] : null);
});
