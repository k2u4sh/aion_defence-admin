...
import { ChevronLeftIcon, EditIcon, TrashIcon, BoltIcon, CheckCircleIcon, AlertTriangleIcon, ClockIcon, StarIcon, UserIcon, MessageIcon, FileIcon, DownloadIcon, PlusIcon } from "@/icons";
...
const BidDetailPage = () => {
  ...
  const handleDeleteBid = async () => {
    ...
    try {
      ...
      const data = await response.json();
      if (data.success) {
        // Redirect back to bids list
        window.location.href = "/admin-management/bids";
      }
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{bid.bidName}</h1>
            <p className="text-gray-600 dark:text-gray-400">Bid Details & Seller Responses</p>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg border p-6">
            ...
            <div className="space-y-4">
              <div>
                <label className="text-sm font-medium text-gray-500 dark:text-gray-400">Bid Name</label>
                <p className="text-gray-900 dark:text-white">{bid.bidName}</p>
              </div>
              <div>
                <label className="text-sm font-medium text-gray-500 dark:text-gray-400">Technical Requirements</label>
                <p className="text-gray-900 dark:text-white whitespace-pre-wrap">{bid.technicalRequirements}</p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="text-sm font-medium text-gray-500 dark:text-gray-400">Duration</label>
                  <p className="text-gray-900 dark:text-white">{bid.duration}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500 dark:text-gray-400">Priority</label>
                  <div className="mt-1">{getPriorityBadge(bid.priority)}</div>
                </div>
                <div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg border p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-gray-900 dark:text-white flex items-center gap-2">
                ...
                Seller Responses
                {((bid.sellerResponses && bid.sellerResponses.length > 0) || bid.sellerResponse) && (
                  <Badge color="info">
                    {bid.sellerResponses ? bid.sellerResponses.length : (bid.sellerResponse ? 1 : 0)}
                  </Badge>
                )}
              </h2>
              <div className="flex gap-2">
            ...
            {(bid.sellerResponses && bid.sellerResponses.length > 0) || bid.sellerResponse ? (
              <div className="space-y-4">
                {/* Display sellerResponses (plural) if available */}
                ...
                {/* Display sellerResponse (singular) if available */}
                {bid.sellerResponse && (
                  <div className="border rounded-lg p-4">
                    <div className="flex items-center justify-between mb-3">
                      <div>
                        <p className="font-medium text-gray-900 dark:text-white">
                          {bid.sellerResponse.seller.firstName} {bid.sellerResponse.seller.lastName}
                        </p>
                        {bid.sellerResponse.seller.companyName && (
                          <p className="text-sm text-gray-500 dark:text-gray-400">
                            {bid.sellerResponse.seller.companyName}
                          </p>
                        )}
                        <p className="text-sm text-gray-500 dark:text-gray-400">
                          {bid.sellerResponse.seller.email}
                        </p>
                      </div>
                      <div className="text-right">
                        {getStatusBadge(bid.sellerResponse.status)}
                      </div>
                    </div>
                    
                    {bid.sellerResponse.quotedPrice && (
                      <div className="mb-2">
                        <label className="text-sm font-medium text-gray-500 dark:text-gray-400">Quoted Price</label>
                        <p className="text-lg font-semibold text-green-600 dark:text-green-400">
                          ${bid.sellerResponse.quotedPrice.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')}
                        </p>
                      </div>
                    )}
                    
                    {bid.sellerResponse.estimatedDelivery && (
                      <div className="mb-2">
                        <label className="text-sm font-medium text-gray-500 dark:text-gray-400">Estimated Delivery</label>
                        <p className="text-gray-900 dark:text-white">{bid.sellerResponse.estimatedDelivery}</p>
                      </div>
                    )}
                    
                    {bid.sellerResponse.notes && (
                      <div className="mb-2">
                        <label className="text-sm font-medium text-gray-500 dark:text-gray-400">Notes</label>
                        <p className="text-gray-700 dark:text-gray-300 text-sm">{bid.sellerResponse.notes}</p>
                      </div>
                    )}
                    
                    <div className="text-xs text-gray-500 dark:text-gray-400">
                      Responded: {formatDate(bid.sellerResponse.respondedAt)}
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="text-center py-8">
                ...
                <h3 className="mt-2 text-sm font-medium text-gray-900 dark:text-white">No seller responses yet</h3>
                <p className="mt-1 text-sm text-gray-500 dark:text-gray-400 mb-4">
                  No sellers have responded to this bid yet. You can add a response manually or wait for sellers to respond.
                </p>
                <Button 
                  variant="outline" 
                  size="sm"
